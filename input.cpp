...
node *InputText(buffer *buf, node *cur_node)
{
    ...
    RenderCharacterAt(buf, cur_node, buf->cursor.line, buf->cursor.column - 1, strlen(cur_node->data), characters_texture, buf->panel.texture);
...
node *InputReturn(buffer *buf, node *cur_node)
{
    if (buf->cursor.column == strlen(cur_node->data))//cursor at end of line
    {
        ...
        {
            RenderClearLine(buf, cur_node, buf->cursor.line, characters_texture, buf->panel.texture);
        }
    }
    else// cursor at start or middle of line
    {
        ...
        RenderClearLine(buf, cur_node->prev, buf->cursor.line, characters_texture, buf->panel.texture);
...
node *InputBackspace(buffer *buf, node *cur_node)
{
    ...
    RenderClearCharacterAt(buf, cur_node, buf->cursor.line, buf->cursor.column, strlen(cur_node->data),characters_texture, buf->panel.texture);
    buf->cursor.last_hor_pos = buf->cursor.column;
